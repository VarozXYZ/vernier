...
type PreflightCostSource interface {
	Snapshot(
		arbitrage.Direction,
		time.Time,
...
type SwapDriver struct {
	...
	PreflightCosts  PreflightCostSource
...
func (d *SwapDriver) ExecuteStage(
	...
	if err := journal.RecordPreparedTransaction(ctx, executionport.PreparedTransaction{
		...
		Phase: "swap", Identity: prepared.Identity, PreparedAt: prepared.PreparedAt,
	}); err != nil {
		return execution.SequentialStageSettlement{}, executionport.NewStageError(
			executionport.DispositionRejected, err,
		)
	...
	if err != nil {
		...
		if broadcast.Disposition == chainport.BroadcastRejected {
			...
			_ = journal.MarkTransaction(
				...
				request.Stage.Ordinal, "swap", "rejected",
			)
		} else {
			_ = journal.MarkTransaction(
				context.WithoutCancel(ctx), request.Operation,
				request.Stage.Ordinal, "swap", "outcome_unknown",
			)
		}
		return execution.SequentialStageSettlement{},
	...
	if err := journal.MarkTransaction(
		ctx, request.Operation, request.Stage.Ordinal, "swap", "broadcast",
	); err != nil {
		return execution.SequentialStageSettlement{},
			executionport.NewStageError(executionport.DispositionPossible, err)
	}
	...
	if settlement.Technical != execution.StateConfirmedSuccess ||
		...
		return execution.SequentialStageSettlement{},
			executionport.NewStageError(executionport.DispositionConfirmedFailure, err)
	}
	...
	if err := journal.MarkTransaction(
		ctx, request.Operation, request.Stage.Ordinal, "swap", "confirmed",
	); err != nil {
		return execution.SequentialStageSettlement{},
			executionport.NewStageError(executionport.DispositionPossible, err)
	}
...
func (d *SwapDriver) Preflight(
	...
	forcedExecution := IsForcedOpportunity(plan.Opportunity)
	if !forcedExecution {
		if err := d.validatePreflightEconomics(
			plan,
			roundTrip,
			clock().UTC(),
		); err != nil {
	...
	d.write(
		"live_preflight operation=%s status=accepted forced_execution=%t buy_input_units=%s buy_output_units=%s sell_input_units=%s sell_output_units=%s sell_preflight_reference=%s round_trip_units=%s latency=%s\n",
		operation,
		forcedExecution,
		plan.InitialInput,
		buy.artifact.ValidatedQuote.AmountOut,
		...
		roundTrip,
		clock().Sub(started).Round(10*time.Microsecond),
	)
	return nil
}

func (d *SwapDriver) validatePreflightEconomics(
	plan execution.SequentialPlan,
	roundTrip market.TokenAmount,
	at time.Time,
) error {
	if d.QuoteAsset == "" || d.PreflightCosts == nil {
		return fmt.Errorf(
			"fresh simulated round trip has no complete-flow cost evidence",
		)
	}
	cost, ok := d.PreflightCosts.Snapshot(
		plan.Opportunity.Direction,
		at,
	)
	if !ok || cost.Amount.Asset() != d.QuoteAsset {
		return fmt.Errorf(
			"fresh simulated round trip has no valid complete-flow cost snapshot",
		)
	}
	...
	gross, err := output.Sub(initial)
	if err != nil {
		return err
	}
	net, err := gross.Sub(cost.Amount)
	if err != nil {
		return err
	}
	minimum, err := market.NewAssetQuantity(
		d.QuoteAsset,
		cloneRatOrZero(d.MinimumNet),
	)
	if err != nil {
		return err
	}
	if net.Rat().Cmp(minimum.Rat()) < 0 {
		return fmt.Errorf(
			"fresh simulated round trip is below net threshold: gross=%s cost=%s net=%s minimum=%s",
			gross,
			cost.Amount,
			net,
			minimum,
