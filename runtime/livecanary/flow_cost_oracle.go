...
// Observe stores the latest complete route quotes and schedules an immediate
// out-of-band refresh. It performs no provider request and never blocks the
// Research event loop.
func (o *FlowCostOracle) Observe(opportunities []arbitrage.Opportunity) {
	o.mu.Lock()
	o.latest = cloneOpportunities(opportunities)
	warm := o.lastAttempt.IsZero()
	o.mu.Unlock()
	if !warm {
		return
	}
...
// I/O and rejects stale data instead of falling back to a configured constant.
func (o *FlowCostOracle) Snapshot(
	...
	o.mu.RUnlock()
	if !ok || cached.snapshot.CapturedAt.IsZero() ||
...
func (o *FlowCostOracle) Components(
	...
	o.mu.RLock()
	components := cloneFlowCostComponents(o.cache[direction].components)
	o.mu.RUnlock()
...
func (o *FlowCostOracle) refreshOnce(ctx context.Context) error {
	...
	byDirection := make(map[arbitrage.Direction]FlowCostEstimate, len(estimates))
	for _, estimate := range estimates {
		if _, duplicate := byDirection[estimate.Direction]; duplicate {
			err = fmt.Errorf("complete-flow refresh repeated a direction")
			o.setRefreshError(err)
			return err
		}
	...
	now := o.clock().UTC()
	next := make(map[arbitrage.Direction]cachedFlowCost, len(o.directions))
	for _, direction := range o.directions {
		...
		for _, component := range estimate.Components {
			if component.Kind == "" || component.Evidence == "" ||
		...
		next[direction] = cachedFlowCost{
			snapshot: arbitrage.CostSnapshot{
				ID: fmt.Sprintf(
					"complete-flow/%s/%s/%d",
					direction.BuyMarket, direction.SellMarket, now.UnixNano(),
				),
				Amount: amount, CapturedAt: capturedAt,
			},
			components: cloneFlowCostComponents(estimate.Components),
		}
	}
	o.mu.Lock()
	...
	for direction, cached := range next {
		...
		o.logger.Info(
			...
			"buy_market", direction.BuyMarket,
			"sell_market", direction.SellMarket,
